...
$block_pos = array();
$max_y = 0;
foreach(Set::extract('/Group/custom_point',$list) as $idx => $xy){
	list($x,$y) = explode(',',$xy);//x,yを配列に分解
	$block_pos[$idx] = array('x'=>$x,'y'=>$y);
	if($max_y < $y) $max_y = $y;//全Groupユニットy座標のうち、最大値を求める
}

//TeX用座標に変換した対応配列を作成
foreach($block_pos as $idx => $pos){
	$pos['x'] = (int)$pos['x'] * TEX_POINT_OCUPY_X;
	$pos['y'] = (int)($max_y - $pos['y']) * TEX_POINT_OCUPY_Y;
	$block_pos[$idx] = implode(',',$pos);
}
?>
